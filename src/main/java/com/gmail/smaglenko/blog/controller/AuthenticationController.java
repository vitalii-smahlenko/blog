...
import com.gmail.smaglenko.blog.security.jwt.JwtTokenProvider;
...
public class AuthenticationController {
    ...
    @PostMapping("/login")
    public ResponseEntity<Object> login(@RequestBody UserAuthRequestDto dto) throws AuthenticationException {
        UserAuth userAuth = authenticationService.login(dto.getUsername(), dto.getPassword());
        String token = jwtTokenProvider.createToken(userAuth.getUsername(), userAuth.getRoles().stream()
                .map(Enum::toString)
